...
const signToken = id => {
  ...
  const createSendToken =(user, statusCode, res) => {
    ...
    res.cookie('jwt', token, cookieOptions); //Send token to client in cookie to secure it
  
    // // Remove password from output
    user.password = undefined;
    //console.log('send token to front end',token);
    res.json(token);
  };
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    //const resetURL = `${req.protocol}://${req.get( 'host')}/api/v1/users/resetPassword/${resetToken}`;
      const link = "http://localhost:4200/pages/resetpassword?token=" + resetToken + "&email=" + user.email;
...
exports.resetPassword = catchAsync(async (req, res, next) => {
    ...
    const hashedToken = crypto
      ...
      .digest('hex');
   // *) Check with passwordResetToken bcoz we dont have any other data for user
   // *)Check if reset link dont expired
    ...
    user.passwordConfirmation = req.body.credentials.passwordConfirmation;
    user.passwordResetToken = undefined;
    user.passwordResetExpires = undefined;
    await user.save();
  ...
  //work if user is logged in only
